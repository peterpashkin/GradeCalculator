<!DOCTYPE html>
<html lang="en">

<head>

  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <meta http-equiv="X-UA-Compatible" content="ie=edge">
  <title>Document</title>
</head>

<body>

<div class="svgcontainer">


  <svg width="100px" height="100px">
    <rect width="50px" height="50px" cx="25px" cy="25px">
    </rect>
  </svg>
</div>
  <?php
  include("datenbank.php");
  session_start();
  /*$benutzer="Pedda";
  include("datenbank.php");
  $query = "SELECT Fach,Benutzername FROM faecher WHERE Pruefungsfach=? AND Benutzername=?";
  $z = $db->prepare($query);

  $z->execute([1,$benutzer]);

if( $y = $z->fetch(PDO::FETCH_BOTH))
{
  echo $y[0] . $y["Benutzername"];
}
*/
/*$name="fgdg";
$sql="SELECT faecher.Fach,fachhalbjahre.Halbjahr,fachhalbjahre.Note FROM faecher,fachhalbjahre WHERE faecher.Fach=fachhalbjahre.Fach AND faecher.Benutzername=?";
$x=$db->prepare($sql);
$x->execute([$name]);

$y=$x->fetchAll(PDO::FETCH_BOTH);
print_r($y);
echo "<br>";
echo $y[1][2];


$arr=[12,14,6,12];

for($i=0;$i<count($y);$i++)
  {
    $fach=$y[$i][0];
    echo $fach;
    $halbjahr=$y[$i][1];
    echo $halbjahr;
    $note=$arr[$i];
    $query="UPDATE fachhalbjahre SET Note=? WHERE Fach=? AND Halbjahr=?";
    $g=$db->prepare($query);
    $g->execute([$note,$fach,$halbjahr]);
  }

*/
/*$grades=[];
$name="Pedda";
$query = "SELECT Fach FROM faecher WHERE Pruefungsfach=? AND Benutzername=?";
$z = $db->prepare($query);

$z->execute([1,$name]);

$fach = $z->fetch(PDO::FETCH_BOTH);


for($x=0;$x<=4;$x++)
{
$fach1=$fach[$x];
for($i=1;$i<=4;$i++)
{
  $sql="SELECT note from Fachhalbjahr WHERE Benutzername=?,Halbjahr=?,Fach=?";
  $f = $db->prepare($sql);
  $f->execute([$name,$i,$fach1]);
  $noten= $f->fetch(PDO::FETCH_BOTH);
  $grades[]=$noten;
}
}*/

$punkteverteilung=[900,822,804,786,768,750,732,714,696,678,660,642,624,606,588,570,552,534,516,498,480,462,444,426,408,390,372,354,336,318,300];
/*
$grades=[];
$name="Pedda";
$query = "SELECT Fach FROM faecher WHERE Pruefungsfach=? AND Benutzername=?";
$z = $db->prepare($query);

$z->execute([1,$name]);


  $fach = $z->fetchAll(PDO::FETCH_BOTH);
  foreach($fach as $f)
    {
      foreach($f as $c)
      {echo "$c ";}
    }

print_r($fach);

echo "<br>" . $fach[1][0];


*/

$s_id=$_SESSION["S_ID"];

//alle Noten des Schülers bestimmen mit Leistungskurs, damit diese in der Rechnung beachtet werden kann
$name=$_SESSION["benutzer"];
$sql="SELECT fachhalbjahr.Note,s_f_beziehung.Leistungskurs,s_f_beziehung.Pruefungsfach from fachhalbjahr,s_f_beziehung WHERE s_f_beziehung.S_ID=? AND fachhalbjahr.S_ID=? AND s_f_beziehung.F_ID=fachhalbjahr.F_ID ORDER BY fachhalbjahr.F_ID DESC";
$f=$db->prepare($sql);
$f->execute([$s_id,$s_id]);
$x=$f->fetchAll(PDO::FETCH_BOTH);

// rechnet die Halbjahre zusammen und rechnet Leistungskurse doppelt
$count=0;
$sum=0;
for($u=0;$u<count($x);$u++)
  {

    if($x[$u][1]==1)
      {
        $count++;
        $sum=$sum+2*$x[$u]["Note"];
      }
    else
      {
        $sum+=$x[$u]["Note"];
      }

  }

//Rechnung der Punkte von Block 1
  $points=round(($sum/(count($x)+$count))*40);


//alle prüfungsfächer bestimmen
  $pruef="SELECT fach.Fachname from fach,s_f_beziehung WHERE s_f_beziehung.S_ID=? AND s_f_beziehung.Pruefungsfach=1 AND fach.F_ID=s_f_beziehung.F_ID";
  $noten=$db->prepare($pruef);
  $noten->execute([$s_id]);
  $u = $noten->fetchAll(PDO::FETCH_BOTH);

//array vorbereiten für sepichern von Noten und alle Prüfungsnoten bestimmen
  $speicherstat=[];
  $sqlueber="SELECT Pruefungsnote FROM fach,pruefung WHERE pruefung.S_ID=? AND fach.F_ID=pruefung.F_ID AND fach.Fachname=?";
  $z=$db->prepare($sqlueber);

//Noten für das jeweilige Fach in das vorbereitete Array packen, dabei Stellen ohne Note mit dem Namen des Faches auffüllen
  foreach($u as $fach)
    {
      $z->execute([$s_id,$fach["Fach"]]);
      $ergebnis=$z->fetch(PDO::FETCH_BOTH);
      if($ergebnis[0]=="")
        {
          $speicherstat[]=$fach["Fach"];
        }
      else {
        //als int definieren, da es sonst Probleme gibt
        $speicherstat[]=(int)$ergebnis[0];
      }
    }

//berechnet und rundet alle Noten eines Arrays
    function average($note)
      {
        $sum=0;
        foreach($note as $a)
          {
            $sum+=$a[0];
          }
          $aver=round($sum/count($note));
          return $aver;

      }

//SQL Syntax für Bestimmung von den Halbjahresnoten falls es keine Note gibt
    $average="SELECT fachhalbjahr.Note from fachhalbjahr,fach WHERE fachhalbjahr.S_ID=? AND fachhalbjahr.F_ID=fach.F_ID AND fach.Fachname=?";
    $b=$db->prepare($average);
    $sum=0;

//Punkte ausrechnen für gegebene Noten und Durchschnitt für Fächer die keinen haben Durchschnittsbildung
    foreach($speicherstat as $s)
      {
        if(is_string($s))
          {
            $b->execute([$s_id,$s]);
            $note=$b->fetchAll(PDO::FETCH_BOTH);

            if(count($note)>0)
            {
            $durchschnitt=average($note);

            $sum+=$durchschnitt*4;
            }
            else {
              $sum+=44;
            }
          }
        else {

          $sum+=$s*4;
        }
      }

//gesamte punkte ausrechnen und mit dem Punkteverteilung Array die resultierende Note bestimmen
    $gesamt=$points+$sum;
    $note=1;
    foreach($punkteverteilung as $p)
      {
        if ($gesamt>$p) {
          break;
        }
        else {
          $note+=0.1;
        }
      }
      echo $note;

//Funktion die restliche Noten mit 15 auffüllt und Durchschnitt ausrechnet
function averagebest($arr)
  {
    $speicher=0;
    $summe=0;
  if(count($arr)!=4)
    {
      for($i=count($arr);$i<4;$i++)
        {
          $speicher+=15;
          $count2++;
        }
    }
    for($a=0;$a<count($arr);$a++)
      {
        if($arr[$a][1]==0)
          {
            continue;
          }
        $summe+=$arr[$a][0];
      }
    $summe+=$speicher;
    return $summe;
  }

//Best-Case Szenario

//alle fächer des Benutzers bestimmen
$fachsyntax="SELECT fach.Fachname,s_f_beziehung.Leistungskurs from fach,s_f_beziehung WHERE s_f_beziehung.S_ID=? AND fach.F_ID=s_f_beziehung.F_ID";
$b=$db->prepare($fachsyntax);
$b->execute([$s_id]);
$allefaecher=$b->fetchAll(PDO::FETCH_BOTH);

//Sytax für alle Noten für das jeweilige Fach vorbereiten
$indinoten="SELECT fachhalbjahr.Note,fachhalbjahr.Einbringen from fachhalbjahr,fach WHERE fachhalbjahr.S_ID=? AND fach.F_ID=? AND fachhalbjahr.F_ID=fach.F_ID";
$h=$db->prepare($indinoten);

$pointsbest=0;
$gegen=0;
foreach($allefaecher as $fach2)
  {
    $h->execute([$s_id,$fach2["Fach"]]);
    $belegtesfach=$h->fetchAll(PDO::FETCH_BOTH);
    //Anzahl der abgewählten Fächer für spätere Rechnung ermitteln
    foreach($belegtesfach as $b)
      {
        if($b[1]==0)
          {
            $gegen++;
          }
      }
    $avarage=averagebest($belegtesfach);
    if($fach2[1]==1)
      {
        $avarage*=2;
      }
    $pointsbest+=$avarage;
  }
  echo $counts2;
  $pointsblockI=($pointsbest/(56-$gegen))*40;
  echo " $pointsblockI";

  //Prüfungsnoten zusammenrechnen und für keine 15 einsetzen

  $indipruef="SELECT Pruefungsnote FROM pruefung WHERE S_ID=?";
  $ax=$db->prepare($indipruef);
  $ax->execute([$name]);
  $pruefnoten=$ax->fetchAll(PDO::FETCH_BOTH);
$pointsblockII=0;
  foreach($pruefnoten as $p)
    {
      $pointsblockII+=$p[0]*4;
    }

  if(count($pruefnoten)<4)
    {
      for($k=count($pruefnoten);$k<=4;$k++)
        {
          $pointsblockII+=60;
        }
    }
  $gesamtbest=round($pointsblockI+$pointsblockII);
  echo " $gesamtbest ";



   ?>


  <form method="POST">
    <label for="arr[]">Prüfungsfach 1/Leistungkurs 1</label>
    <select name="P1" id="arr[]">
      <option value="Physik">Physik</option>
      <option value="Chemie">Chemie</option>
      <option value="Biologie">Biologie</option>
      <option value="Geschichte">Geschichte</option>
      <option value="Kunst">Kunst</option>
      <option value="Englisch">Englisch</option>
      <option value="Französisch">Französisch</option>
    </select>
    <br>
    <label for="arr[]">Prüfungsfach 2/Leistungskurs 2</label>
    <select name="arr[]" id="arr[]">
      <option value="Mathe">Mathe</option>
      <option value="Deutsch">Deutsch</option>
    </select>
    <input type="submit" name="" value="">
  </form>
  <?php
$arr=$_POST["arr"];

 ?>
</body>

</html>
